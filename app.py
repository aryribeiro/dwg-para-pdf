...
# ---------------------------------------------------------------------------
st.set_page_config(
    page_title="Conversor DWG para PDF",
    page_icon="📐",
    layout="centered",
...
# ---------------------------------------------------------------------------
# CONVERSÃO: DWG (AutoCAD) -> PDF
# Escopo único e fixo — este app não lida com nenhum outro par de formatos.
# Cadeia: dwg2dxf (GNU LibreDWG, binário em bin/) -> ezdxf -> PyMuPDF -> PDF.
# O desenho entra no PDF como VETOR (linhas e curvas), não como uma imagem
# dentro de um PDF: ampliar não perde nitidez e o texto continua texto.
# ---------------------------------------------------------------------------
SOURCE_EXT = ".dwg"
TARGET_EXT = "pdf"
TARGET_MIME = "application/pdf"

# Página do PDF: A4 na orientação do desenho.
A4_LONG_MM = 297.0
A4_SHORT_MM = 210.0
PAGE_MARGIN_MM = 10.0
...
def get_conversion_slots():
    ...
    Um DWG grande leva o dwg2dxf a centenas de MB e a montagem do PDF
    guarda o desenho inteiro em memória; sem este limite, N usuários =
    N vezes isso e o container do Streamlit Cloud (1 GB) morre por falta
    de memória."""
...
# a coleção em static/fonts como reserva; o packages.txt pede as fontes
...
# Classes que o LibreDWG deixa de escrever mas que nunca aparecem no papel
...
# ---------------------------------------------------------------------------
# ETAPA 2: DXF -> PDF COM ezdxf + PyMuPDF
# ---------------------------------------------------------------------------
def page_for_extents(width: float, height: float):
    """Devolve (página_do_PDF, rótulo) para um desenho com estes limites.

    Por que A4, e não a página de 10 polegadas do app irmão que gera PNG:
    lá o número existia só para chegar a 4000 pixels no lado maior — o PNG
    tem resolução, o PDF não tem. Aqui o desenho vai para o papel como
    vetor, então o tamanho da página não decide nitidez: decide folha.
    A4 é a folha que todo mundo tem em casa e no escritório, e é o padrão
    que qualquer visualizador e qualquer impressora entendem sem ajuste.
    A orientação acompanha o desenho (deitado ou em pé) para o desenho
    ocupar o máximo do papel, e o encaixe é proporcional (fit_page): o
    desenho entra inteiro, sem corte e sem distorção. Quem quiser plotar
    em A1 amplia na impressora — por ser vetor, não perde nada."""
    width = max(width, 1e-9)
    height = max(height, 1e-9)
    if width >= height:
        w_mm, h_mm = A4_LONG_MM, A4_SHORT_MM
        rotulo = f"A4 paisagem ({A4_LONG_MM:.0f} × {A4_SHORT_MM:.0f} mm)"
    else:
        w_mm, h_mm = A4_SHORT_MM, A4_LONG_MM
        rotulo = f"A4 retrato ({A4_SHORT_MM:.0f} × {A4_LONG_MM:.0f} mm)"
    page = layout.Page(
        w_mm, h_mm, layout.Units.mm,
...
def dxf_to_pdf(dxf_path: Path):
    """Devolve (pdf_bytes, info). info = versão do DXF, entidades, camadas,
    página e fontes."""
    ...
    # parágrafo quebra numa linha a mais e as linhas se atropelam. Aqui o
    # estrago é permanente, porque o texto errado vai gravado dentro do PDF.
    # Só aparece em servidor que TEM fontes de sistema — por isso passava na
    # bancada. Foi o defeito da v1.1.2 do app irmão de PNG.
    ...
    if not extents.has_data:
        st.error("❌ O desenho não tem nenhum elemento visível no espaço do modelo "
                 "(linhas infinitas, hélices e sólidos 3D não viram desenho no papel).")
        raise ConversionError("desenho vazio")

    page, page_label = page_for_extents(extents.size.x, extents.size.y)
    ...
    pdf = backend.get_pdf_bytes(page, settings=layout.Settings(fit_page=True))
    if not pdf or not pdf.startswith(b"%PDF-"):
        st.error("❌ O PDF não pôde ser gerado. O desenho pode ser grande demais.")
        raise ConversionError("pdf vazio")
...
def render_preview(pdf_bytes: bytes, dpi: int = PREVIEW_DPI):
    """Imagem da 1ª página do PDF QUE SERÁ BAIXADO. A prévia sai do próprio
    arquivo entregue, e não de um segundo desenho feito por outro caminho:
    assim o que a pessoa vê na tela é o artefato, não uma aproximação dele."""
...
# PIPELINE COMPLETO
# ---------------------------------------------------------------------------
def convert_dwg_to_pdf(input_file: str):
    """Converte um .dwg em PDF. Devolve (pdf, prévia_png, info). Pasta de
    trabalho própria por chamada e apagada no finally, mesmo em erro."""
    ...
    try:
        ...
        with get_conversion_slots():
            ...
            pdf, info = dxf_to_pdf(dxf_path)
            preview = render_preview(pdf)
...
def convert_upload_to_pdf(file_name: str, file_bytes: bytes):
    """Converte com cache por conteúdo: clicar em "Baixar PDF" dispara um
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        return convert_dwg_to_pdf(str(input_path))
...
def main():
    ...
    with st.spinner(f"Convertendo para {TARGET_EXT.upper()}..."):
        try:
            pdf_bytes, preview_png, info = convert_upload_to_pdf(
    ...
    detail = (f"AutoCAD {info['dwgversion']} · {info['entities']} elementos · "
              f"{info['layers']} camadas · página {info['page']}")
    if info.get("fonts"):
    ...
    if info["unknown_objects"] or info["skipped_classes"]:
        ...
        st.warning("⚠️ Leitura parcial: " + "; ".join(parts) + ". Confira se falta algo no PDF.")
